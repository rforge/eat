ctStanFit<-function(datawide, ctmodelobj, continuoustime=TRUE, kalman=TRUE, densehyper=FALSE,
  indvarying='all', fit=TRUE, plot=FALSE,noncentered=FALSE,
  ...
  ctspec$transform[ctspec$matrix %in% c('T0MEANS','MANIFESTMEANS','TDPREDEFFECT','CINT') & freeparams] <- '(param) * 10'
...
paste0(unlist(lapply(1:nrow(ctspec),function(rowi){
  ...
  if(fit==TRUE){
    ...
    standata<-list(
      ...
      nobs_y=array(apply(datalong[,ctmodelobj$manifestNames,drop=FALSE],1,function(x) length(x[x!=99999])),dim=nrow(datalong)),
      whichobs_y=matrix(t(apply(datalong[,ctmodelobj$manifestNames,drop=FALSE],1,function(x) {

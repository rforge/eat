...
#
# 2011-12-28 SW
# FIXED: as.numeric.if.possible() now allows for vectors and logical columns
# 0000-00-00 AA
#
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
as.numeric.if.possible <- function(dataFrame, set.numeric=TRUE, transform.factors=FALSE, maintain.factor.scores = TRUE, verbose=TRUE)   {
            funVersion  <- "as.numeric.if.possible_0.4.0"
            originWarnLevel <- getOption("warn")
            wasInputVector  <- FALSE
            if(class(dataFrame) != "data.frame" ) {
              if(verbose == TRUE) {cat(paste(funVersion, ": Warning! Argument of 'as.numeric.if.possible' has to be of class 'data.frame'. Object will be converted to data.frame.\n",sep=""))}

...
.automateModels.conquest.multigroup <- function ( model.specs , dataset ) {
		...
		.fun <- function ( software , person.grouping , p.model ) {
				...
				if ( software == "conquest" & ncol ( person.grouping ) > 2 ) {
						...
						# TODO
						# sunk ( paste ( f.n , "VORSICHT: noch nich implementiert ist, Gruppen der gleichen Variable zu mergen\n" ) )
						sunk ( paste ( f.n , "DANGER: Merging several Groups of the same Variable is not yet supported\n" ) )
						...
						if ( ! all ( group.name.check ) ) {
								...
								# sunk ( paste ( f.n , "Gruppen-Namen" , string , "entsprechen nicht der Konvention <var>.<cat>\n" ,
													# "und werden deshalb für die Erzeugung des group statement für ConQuest nicht verwendet.\n" ) )
								sunk ( paste ( f.n , "group names" , string , "are not consistent with convention <var>.<cat>\n" ,
						...
						if ( ! any ( colnames ( dataset ) %in% group ) ) {
								sunk ( paste ( f.n , "\n" ) )
								# sunk ( paste ( "Variable(n)" , paste ( group , collapse = ", " ) , "ist/sind nicht im Datensatz.\n" ) )
								# sunk ( paste ( "Bitte Datensatz und/oder person.grouping anpassen.\n" ) )								
								sunk ( paste ( "Variable(s)" , paste ( group , collapse = ", " ) , "is/are not in dataset.\n" ) )
								sunk ( paste ( "Please check dataset and/or person.grouping\n" ) )
								stop()
						}

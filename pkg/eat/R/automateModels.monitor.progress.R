...
.automateModels.monitor.progress <- function( FolderList , folder.out , analyse.name , software, refresh=5 , time.out=240, 
	...
	sunk("automateModels.monitor.progress: Start monitoring ... \n")
	
	# 14.10.2011 MH: Paket wird schon früher gesourct, deshalb auskommentiert
	# if(!is.null(email)) { 
		# if(inherits( try( library(sendmailR)) ,"try-error")){
		# sunk("Es wird keine Email kommen, da das Paket sendmailR nicht geladen werden konnte.\n")
		# sunk("Package sendmailR could not be loaded.\n")
		# }
	# }	
			...
			if(software[[i]] == "conquest") {	
				...
				if (!identical(dir(check.folder), character(0))) {
					if(any( grep( "laji", dir(check.folder) ) ) ) {
						ret[[i]] <- FALSE
						# sunk(tr[i+a,1] <- paste(Sys.time(), ": Die Conquest-Analyse \'",analyse.name[[i]],"\' in: \'", FolderList[[i]], "\' läuft noch. \n", sep =""))
						if(communicated1[[i]] == FALSE) {
							sunk(tr[i+a,1] <- paste(Sys.time(), ": Conquest-Analysis \'",analyse.name[[i]], "\' is running. In: \'", check.folder, "\' \n", sep =""))
							communicated1[[i]] <- TRUE
						}
					} else {
						if (any( paste(analyse.name[[i]], ".shw", sep ="") %in% dir(check.folder ) ) ) {
							ret[[i]] <- TRUE
							# sunk(tr[i+a,1] <- paste(Sys.time(), ": Die Conquest-Analyse \'",analyse.name[[i]],"\' in \'", FolderList[[i]], "\' ist fertig !!! \n", sep =""))
							sunk(tr[i+a,1] <- paste(Sys.time(), ": Conquest-Analysis \'",analyse.name[[i]], "\' is done !!! In: \'", check.folder, "\' \n", sep =""))
						} else {
							ret[[i]] <- FALSE
							if(communicated2[[i]] == FALSE) {
							# sunk(tr[i+a,1] <- paste(Sys.time(), ": Die Conquest-Analyse \'",analyse.name[[i]],"\' in \'", FolderList[[i]], "\' wurde noch nicht gestartet. \n", sep ="")) #\n Weder \'laji...\' noch \'.shw\' auffindbar.
								sunk(tr[i+a,1] <- paste(Sys.time(), ": Conquest-Analysis \'",analyse.name[[i]],"\' has not been started yet. In: \'", check.folder, "\' \n", sep ="")) #\n Weder \'laji...\' noch \'.shw\' auffindbar.
								communicated2[[i]] <- TRUE
							}
						}
					}	
				} else { ret[[i]] <- FALSE
						 # sunk( tr[i+a,1] <-  paste(Sys.time(), ": Das Verzeichnis: \'", FolderList[[i]], "\' ist leer. \n", sep =""))}
						 sunk( tr[i+a,1] <-  paste(Sys.time(), ": Folder: \'", check.folder, "\' is not empty. \n", sep =""))}
			} else {
				sunk(paste("automateModels.monitor.progress: monitoring for other software than ConQuest still not implemented yet."))
			}
		flush.console()
		}
		a <- a + length(FolderList) #[!unlist(ret)]		
		
		# if (all(ret == TRUE)) {sunk(paste(Sys.time(), ": Alle Analysen wurden erfolgreich abgeschlossen! \n", sep =""))		
		if (all(ret == TRUE)) {sunk(paste(Sys.time(), ": All analyses terminated successfully! \n", sep =""))		
								if(!is.null(email)) {
									...
									# sunk(paste("automateModels.monitor.progress: Fehler beim versenden der Email.\n",sep=""))}
									sunk(paste("automateModels.monitor.progress: Error while sending email.\n",sep=""))}
								}
								return(ret)
								break}		

		z <- round((proc.time()-p1)[[3]]/60, 1)
		# if (z > time.out) {sunk( paste(Sys.time(), ": Time-Out erreicht!! Monitoring wird abgebrochen. \n", sep =""))
		if (z > time.out) {sunk( paste(Sys.time(), ": Time-Out!! Monitoring aborted. \n", sep =""))
							if(!is.null(email)) {
									tr <-  tr[-which(is.na(tr)),]

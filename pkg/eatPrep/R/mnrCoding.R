...
mnrCoding <- function ( dat , pid , rotation.id , blocks , booklets , breaks , subunits = NULL , nMbi = 2 , mbiCode = "mbi" , mnrCode = "mnr" , invalidCodes = c ( "mbd", "mir", "mci" ) , verbose = TRUE ) {
		...
		if ( verbose ) {
				cat ( "identifying items in data (reference is blocks$subunit)\n" )
		}
		# colnames
		cn <- colnames ( dat ) 
		cn1 <- cn %in% blocks$subunits
		check1 <- all ( cn1 )
		if ( !check1 ) {
				...
				if ( !is.null ( subunits ) ) {
						...
						if ( check2 ) {
								...
								# check ob Originalnamen in blocks$subunit
		...
		if ( verbose ) {
				cat ( paste ( "identifying items with no mbi-codes ('" , mbiCode , "'):\n" , sep = "" ) )
		...
		fun <- function ( abschn , dat , rotation.id , nMbi , mbiCode , pid , verbose , invalidCodes ) {
				...
				if ( !is.null ( x ) ) {
						...
						fun3 <- function ( items , id , durchgang , pid ) {
								da <- data.frame ( "item" = items , stringsAsFactors = FALSE )
								da$pid <- id
								colnames(da)[2] <- pid
								return ( da )
						}
						da <- mapply ( fun3 , x , names ( x ) , seq(along=x) , MoreArgs = list ( pid ) , SIMPLIFY = FALSE ) 

...
\subsection{Populations means, standard deviations and variances}
\label{subsec:means}
...
executed, which results in $3\times 81=243$ analyses overall. The little dots continuously appearing on console are intended to work as
...
As we might have expected, the outcome is a list of two data frames, according to the two dependent variables. However, please note that
...
\code{INCOME} are estimated. If we would have used \code{gender} as a group variable instead of a predictor variable, the two
...
continuous numeric variable. What we see additionally is that R implicitly defined the female group to be the reference -- the 
...
As we saw in the preceding section, R choosed the refenrence group of factor variables by itself. Persuading R to meet our needs

...
i.e. we split the data and only consider cases for which $nest=1$. We may think of \code{"score"} as the the
...
the subsequent analysis has to be repeated 81 times \emph{for each imputation}. Hence, $3\times 81=243$ analyses are
run overall.\\ 
...
repeated 81 times, \emph{using the same plausible value} as the dependent variable. Only the weights vary between the
...
81 ``cluster replicates''. When finished, the analysis approaches the second ``imputation replicate'' and switches 
to the second plausible value which now is used as the dependent variable in 81 analyses due to the 
81 ``cluster replicates''. After all, three pooled estimates and three pooled standard errors resulted 
...
the output is more suited for further processing than for clear arragement, as the
...
displaying parameters or coefficients you are not interested in at the moment. \\
...
estimates are considerably lower. (Standard errors for standard deviations and for
variances are not implemented yet.) If we decide to leave out the weights as well,
...
Again, we might use the \code{dM} function to display the part of the results we are interested in, but for the 
representation within this pdf document, an alternative way is more clearly arranged. Note that now we do \emph{not} exclude 
\code{meanGroupDiff} from the results to summarize:\\

<<readingMeansByCountry5def>>=
### 'conventional' representation using 'dM'
results1 <- dM(means, omitTerms = c("var","Ncases", "NcasesValid"))
### alternative representation
results2 <- means[grep("^mean", means[,"parameter"]) ,]
results2 <- reshape2::dcast ( results2, group + parameter ~ coefficient, value.var = "value")
...
means <- jk2.mean(datL = readN1, ID = "idstud", wgt = "wgtSTUD", type = "JK2",
         PSU = "JKZone", repInd = "JKrep", imp = "imputation", groups = c("sex","country"),
         group.differences.by = c("sex", "wholePop"), dependent = "score")
### alternative representation
results <- means[grep("^mean|^wholePop", means[,"parameter"]) ,]
results <- reshape2::dcast ( results, group + parameter ~ coefficient, value.var = "value")
...
freqs <- jk2.table( datL = readN1, ID = "idstud", wgt = "wgtSTUD", type = "JK2", 
         PSU = "JKZone", repInd = "JKrep", imp = "imputation", groups = c("country", "sex"), 
	       group.differences.by = "sex", chiSquare = TRUE, dependent = "passed")
...
Unfortunately, the results of the chi square test are not yet captured by the \code{dT} function, 
so we have to extract them from the \code{freqs} object by ourself:\\

<<hiseiFreqsExtraction>>=
options(scipen=4)
cols <- c("group", "depVar", "parameter", "coefficient", "value")
print ( freqs[which(freqs[,"parameter"] == "chiSquareTest"), cols ], digits = 1)
@
...
freqs <- jk2.table( datL = readN1, ID = "idstud", wgt = "wgtSTUD", type = "JK2", 
         PSU = "JKZone", repInd = "JKrep", imp = "imputation", groups = c("country", "sex"), 
	       group.differences.by = "sex", chiSquare = FALSE, dependent = "passed")
result<- dT(freqs, percent = TRUE)
result[,-match ( "depVar", colnames(result))]
@
...
More specifically, for each country, an intercept and two regression coefficients according to \code{gender} and 

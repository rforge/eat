...
into the argument \group.differences.by}:\\
...
We see that the group differences in the whole population are -27.4 points. The output
column \code{wholePopDiff_est} yields the difference for each country from the
``global'' difference. In \code{LandA}, the difference is more substantial than in
the whole population, in \code{LandB} and \code{LandC}, the difference is less
substantial. However, no country-specific difference varies significantly from
the difference in the whole population, as the amount of deviation is less than
twice the standard error of the amount.\\
...
options(scipen=4)
print ( freqs[which(freqs[,"parameter"] == "chiSquareTest"),], digits = 1)
@

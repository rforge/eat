...
res  <- freqs[which(freqs[,"parameter"] == "chiSquareTest"), cols ]
...
### with a little reshaping endeavor
result<- freqs[grep("meanGroupDiff", freqs[,"parameter"]),]
result<- reshape2::dcast(result, country+sex+parameter~coefficient, value.var="value")
result[,"p.value"] <- 2*(1-pnorm ( abs ( result[,"est"] / result[,"se"] ) ))

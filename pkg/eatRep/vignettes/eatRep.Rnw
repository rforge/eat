...
\begin{abstract}
  ...
  the computer software WesVar \citet{Westat2000}---in this case the package only allows for executing
  ...
  \code{eatRep} heavily relies on the \code{survey} package \citet{Lumley2012} which functions has been extended by methods 
...
standard errors, too \citet{luke2009}. This problem occurs in the same way for estimation of frequency tables, 
...
sample is underestimated due to a clustered structure \citet{Lumley2004}. In the Jackknife-2 (JK2) procedure 
...
Several software \citet{Westat2000} and free R packages such as \code{survey} \citet{Lumley2012}
...
cases appear as ``NA'' values, it is possible to define them as a distince category for which 

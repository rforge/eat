...
have to be repeated to account for multiple imputed data. In the following, we refer to \dQuote{cluster replicates} 
and \dQuote{imputation replicates} to differentiate between both.
...
i.e. wo split the data and only cosider cases for which $nest=1$. We may think of \code{"score"} as the the 
...
assumes an \dQuote{unnested}, i.e. a structure with three imputations. This refers to what we've called 
\dQuote{imputation replicates}. The output then speaks about 81 replicate weights which are created due to 81 distinct 
jackknifing zones in the JKZone variable. This information refers to the \dQuote{cluster replicates} and implies that 
...
\dQuote{cluster replicates}: each of the 81 replicates once a time is used as the weighting variable. Each of the 81 
analysis revealed slighty different results. This variation is used to estimate the sampling variance which then is used 
to compute the standard error, which is pooled across 81 \dQuote{cluster replicates}. When finished, the analysis 
approaches the second \dQuote{imputation replicate} and switches to the second plausible value which now is used as the 
dependent variable in 81 analyses due to the 81 \dQuote{cluster replicates}. After all, three pooled estimates and three 
pooled standard errors resulted which differ slightly in each \dQuote{imputation replicate}. The three estimates and 
...
The little dots continuously appearing on the console therefore refer to \dQuote{imputation replicates} and are intended 
to work as a rough progress bar. Each dot represents one \dQuote{imputation replication}. When the procedure finished, 
...
function (whereas dM stands for \dQuote{display means}). You may think of \code{dM} as 
...
a simple indicator, i.e. a categorical variable. Categorical variables are often represented 
as factors in R, which is quite straightforward. However, the \code{"passed"} variable is of 
class numeric. This is an inconsistency which may cause annoying misinterpretations when such 
...
(i.e. \dQuote{passed}). The \dQuote{labels} of the dependent variable now are captured in the 
...
it does not seem plausible to assume missnig values on the \code{"passed"} variable. Without
...
option \dQuote{I don't know my parents' profession.} Conceptually, it makes considerably more 
sense to define a separate category during the data preparation, for example \dQuote{lowest HISEI}, 
\dQuote{medium HISEI}, \dQuote{highest HISEI}, \dQuote{unknown HISEI}. Families without valid
HISEI information then will be considered as a separate group in the analyses. Applying 
\code{jk2.table} then will give frequencies for four groups. However, if the \dQuote{Dont't know} 
cases appear as \dQuote{NA} values, it is possible to define them as a distince category for which 
...
has joined to the output, which is labelled \code{"missing"}. \\
...
Second, we have to specify the regression expression, as in \code{glm()}, for example. Third, we will 
have to specify the kind of regression we propose to estimate, for example linear or logistic regression. 
We start with a simple example using the same data as before.\\
...
speaks of about \dQuote{1 analysis overall} according to two \code{'group.splits = 1'}. 
...
depends on certain independetn variables. Secondly, we also use country as a predictor (instead 
...
is significantly lower for males and for examinees who stem from \code{"LandB"}. Moreover, 
the disadvantage of boys is not consistent across countries: In \code{"LandB"}, this difference
...
before \code{"female"} alphabetically. Consequently, \code{"male"} is used as reference group when repeating 
the last example:\\
...
Weirich et al. (in press) described the same procedure more elaboratively. Suppose you want to estimate IRT proficiencies 
(often denoted $\theta$) in a specific domain. Applying an extensive marginal model which comprehends of items responses and 
...
procedures according to the generalized linear model. To obtain valid estimates and standard errors, the results have to be 
...
stem from a common \sQuote{nest}. The interdependence has to be taken into account. Whereas the conventional pooling formulas 
...
As we only have two nests, only two imputation were created in the first step. Within each of this two imputations, three 
...
(e.g. income) only vary between nests, but not between imputations with each nest! To date, the \code{eatRep} does not provide
...
which denotes the \dQuote{nests}.\\
...
We now want to predict \dQuote{reading ability} by \code{sex} and \code{income}. Using \code{country} 
...
%%large-scale assessments, for example in the \dQuote{Laendervergleich} studies processed at the 
...
%%contain (imputed) values for the corresponding scale (\dQuote{reading ability}) but only the responses
...
%%In the second step, we use WLEs scores obtained from the calibration model as an \dQuote{auxiliary variable}
...
%%each data set. This is realized using a \dQuote{by}-loop. The item parameters from the 

...
an abbreviated display of the results is provided by the \code{summary} function. Note that the \code{summary} method 
implemented in \code{eatRep} is \emph{not} intended for saving results or further processing as the results are displayed
in an abbreviated (i.e., rounded) matter to offer clear arrangement on console. The \code{summary} function has an 
additional argument to omit displaying parameters or coefficients you are not interested at the moment. \\
...
federal state}, gender and federal state has to be part of the \code{groups} statement, whereas only gender has to be 
...
\emph{across each federal state}, only gender has to be part of the \code{group} statement, and only gender has to be 
used in the \code{group.differences.by} argument. The output of the analysis is nearly the same as we would have omitted 
the \code{group.differences.by} argument, but now, some additional lines have joined. Again, we may use the \code{summary} 
...
The output is a single data frame in the long format. Again we use the \code{summary} function to make the output more
pleasing to the eye. For each gender group in each country, the relative frequency
...
For our purpose, it may be sufficient to content ourself with the summary. First let us consider how many regression 
analyses are conducted. The message on the console speaks of about "2 analyses overall" according to two dependent variables,
reading and writing. But strictly speaking, we have estimated \emph{six} regression analyses, as the model is fitted in each
...
to \code{gender} and \code{INCOME} are estimated. The \code{summary} function allows us to have a look only 
at a specific result out of the 6. \code{analyses = 1:2} advises the function to display the results of the first and second
analysis. First we should consider that the each single analyses is characterized by two variables, the group for 
which the model is fitted, and the dependent variable. In the heading we find information about both. The actual 
